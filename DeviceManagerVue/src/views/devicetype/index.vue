<template>
  <div class="app-container">
    ...
    <div class="filter-container" style="margin-bottom: 15px">
      <!-- 类型名输入 -->
      <el-input v-model="queryParam.deviceTypeName" placeholder="类型名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <!-- 一些按钮 -->
    ...
    <el-dialog :title="formTitle" :visible.sync="dialogFormVisible" width="30%">
      ...
      <el-form :model="form" :rules="rules" ref="ruleForm" label-width="80px">
        ...
        <el-form-item label="类型描述" prop="deviceTypeDesc">
          <el-input type="textarea" v-model="form.deviceTypeDesc"></el-input>
        </el-form-item>
      </el-form>
...
import { getCount,queryDeviceTypes,queryDeviceTypesByPage,addDeviceType,deleteDeviceType,deleteDeviceTypes,updateDeviceType }
from '@/api/devicetype'
...
export default {
  ...
  created() {
    // 从服务器获取数据表格第一页的信息
    queryDeviceTypesByPage(this.queryParam).then(res => {
  ...
  methods: {
    ...
    handleSizeChange(curSize) {
      ...
      queryDeviceTypesByPage(params).then(res => {
    ...
    handleShowAll() {
      this.queryParam.page = 1
      this.queryParam.deviceTypeName = null
      queryDeviceTypesByPage(this.queryParam).then(res => {
        if(res.code === 0) {
          this.tableData = res.data
          this.recordTotal = res.count
    ...
    handleDeleteSome() {
      this.$confirm('确定要删除这些记录吗?', '提示', {
        ...
        deleteDeviceTypes(items).then(res => {
          if(res > 0) {
